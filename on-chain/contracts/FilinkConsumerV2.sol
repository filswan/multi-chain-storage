...
contract FilinkConsumer is ChainlinkClient, Ownable {
    ...
    // API variables
    string public filinkEndpoint; // API URL
    string public jsonPath;  // comma separated JSON path ex. data,deal,storage_price
    int256 public priceMultiplier; // multiplier to storage price ex. 10**18 to convert to Wei
    ...
    function setFilinkEndpoint(string memory newEndpoint) public onlyOwner {
        filinkEndpoint = newEndpoint;
    }

    function setJsonPath(string memory newJsonPath) public onlyOwner {
        jsonPath = newJsonPath;
    }

    function setPriceMultiplier(int256 newPriceMultiplier) public onlyOwner{
    ...
    function requestDealInfo(string calldata deal, string calldata network) public returns (bytes32 requestId) {
        ...
        /**
         * GET <filinkEndpoint>/<deal>?network=<network>
         * ex. GET https://flink-adapter.filswan.com/deal/123456?network=filecoin_mainnet, data.deal.storage_price = 8294400600825600
         */ 
        request.add("get", concatenate(filinkEndpoint, params));
        request.add("path", jsonPath);
        request.addInt('times', priceMultiplier);
