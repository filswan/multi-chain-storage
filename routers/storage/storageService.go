...
func GetSourceFileAndDealFileInfo(limit, offset string, userId int) ([]*SourceFileAndDealFileInfo, error) {
	sql := "select s.file_name,s.file_size,s.pin_status,s.create_at,df.miner_fid,df.payload_cid,df.deal_cid,df.piece_cid,df.deal_status,df.deal_status as status from  source_file s " +
