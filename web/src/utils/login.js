...
async function get_nonce() {
    ...
    const baseAPIURL = store.getters.networkID == 97?process.env.BASE_PAYMENT_GATEWAY_BSC_API:store.getters.networkID == 80001?process.env.BASE_PAYMENT_GATEWAY_API:process.env.BASE_PAYMENT_GATEWAY_POLYGON_API
...
async function perform_signin(sig, nonce) {
    const reqOpts = {
        ...
        nonce: nonce,
        signature: sig,
        network: store.getters.networkID == 97 ? "bsc.testnet" : store.getters.networkID == 80001 ? "polygon.mumbai" : "polygon.mainnet"
    }
    const baseAPIURL = store.getters.networkID == 97?process.env.BASE_PAYMENT_GATEWAY_BSC_API:store.getters.networkID == 80001?process.env.BASE_PAYMENT_GATEWAY_API:process.env.BASE_PAYMENT_GATEWAY_POLYGON_API
