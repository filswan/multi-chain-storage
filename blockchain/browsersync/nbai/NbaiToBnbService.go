...
func changeNbaiToBnb() {
	...
	nonce, err := nbaiclient.WebConn.PendingNonceAt(context.Background(), fromAddress)
	if err != nil {
		logs.GetLogger().Error(err)
	}
	gasPrice, err := nbaiclient.WebConn.SuggestGasPrice(context.Background())
	if err != nil {
		logs.GetLogger().Error(err)
	}
	...
	contractAbi, err := abi.JSON(strings.NewReader(string(StateSenderABI)))
}
